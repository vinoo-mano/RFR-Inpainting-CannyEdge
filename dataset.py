...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, image_path, mask_path, mask_mode, target_size, augment=True, training=True, mask_reverse = False):
        ...
        print(f"Loaded {len(self.data)} images and {len(self.mask_data)} masks.")  # Add this line to see the loaded data count
    ...
    def load_list(self, path):
        ...
        if isinstance(path, str):
            if path[-3:] == "txt":
                ...
                file_names = []
                for line in lines:
                    file_names.append("RFR-Inpainting/Places2/train_256_places365standard/data_256"+line.split(" ")[0])
                return file_names
            if os.path.isdir(path):
                ...
                path.sort()
                path = path[0:2500]
                print("Files found:", len(path))  # Add this line
                return path
