...
def canny_edge(image, low_threshold=100, high_threshold=200):
    ...
    return edges
'''
def canny_edge(images, low_threshold=100, high_threshold=200):
    batch_size, _, height, width = images.shape
    edges_batch = np.empty((batch_size, 1, height, width), dtype=np.uint8)

    for i, image in enumerate(images):
        
        image_np = np.array(image).transpose((1, 2, 0))
        gray_image = cv2.cvtColor(image_np, cv2.COLOR_RGB2GRAY)  # Convert to grayscale
        gray_image = np.uint8(gray_image * 255)  # Convert to 8-bit single channel
        blurred_image = cv2.GaussianBlur(gray_image, (5, 5), 0)
        #print("Image depth:", image.shape, image.dtype)
        edges = cv2.Canny(blurred_image, low_threshold, high_threshold)  # Use the converted image
        edges_batch[i, 0] = edges
...
class RFRNet(nn.Module):
    ...
    def forward(self, in_image, mask):
        ...
        # Apply Canny edge detection to in_image
        #edges = canny_edge(image_np)
        edges = canny_edge(in_image.detach().cpu().numpy())
        


        # Convert NumPy array to PyTorch tensor
        edges_tensor = np_to_tensor(edges)

        # Use edges_tensor as input to the RFRNet module
        if in_image is None:
            raise ValueError("in_image argument cannot be None")
